...
void GPControl::timeout()
{
  ...
  if (!isEnd()) {
    nextCar();
    QTimer::singleShot(delay * 20, this, SLOT(nextMove()));
  } else {
    emit end(results());
  }
}
...
void GPControl::nextCar()
{
  int x;
  int y;
  static int carSequence[3] = { 0, 1, 2 };
  static int positionInSequence = 0;
  if (nextCarMode == RANDOM) {
    // Generate a new sequence after 3 steps.
    if ( ! positionInSequence ) {
      carSequence[0] = (int) (rand() % 3);
      do {
        carSequence[1] = (int) (rand() % 3);
      } while (carSequence[0] == carSequence[1]);
      do {
        carSequence[2] = (int) (rand() % 3);
      } while (carSequence[0] == carSequence[2] || carSequence[1] == carSequence[2]);
    }
    do {
      currentCarIndex = carSequence[positionInSequence];
      x = cars[currentCarIndex].positionsX.last();
      y = cars[currentCarIndex].positionsY.last();
      positionInSequence = (positionInSequence + 1) % 3;
